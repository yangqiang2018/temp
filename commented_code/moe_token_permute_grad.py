...
def _is_fp32(dtype: str) -> bool:
    # 判断字符串形式的 dtype 是不是 fp32
    # ("float" 是 numpy/torch 里 fp32 的别名, 这里一并接受)
    return dtype in ("float32", "float")


def _pad_last_dim(tensor: torch.Tensor, target_cols: int) -> torch.Tensor:
    """
    把张量沿最后一维补零到 target_cols.
    若已经够大就直接返回, 不做拷贝.

    注: 本文件实际上没用到这个函数 (留下来是历史遗物 / 公共工具),
    但保留它便于和 unpermute_grad 等姊妹文件保持接口一致.
    """
    if tensor.shape[-1] >= target_cols:
        return tensor  # 不需要 pad, 直接返回原张量 (零拷贝)

    # 创建一个全零的、最后一维等于 target_cols 的新张量
    out = torch.zeros((*tensor.shape[:-1], target_cols), dtype=tensor.dtype, device=tensor.device)
    out[..., : tensor.shape[-1]] = tensor  # 把原内容写到前 N 列, 后面剩零
    return out
            ...
            with T.Kernel(actual_cores, is_npu=True) as (cid, vid):
                ...
                with T.Scope("V"):
                    for batch_id in T.serial(n_batches):
                        ...
                        # ---- prologue: 预取前 7 个 lane 进 row_buf[0..6] ----
                        # (注意只预取 7 个, 第 8 个由主循环的 it=0 那一轮的 prefetch 顺手做)
                        # 写法上 8 个 if 是手动 unroll, 让编译器看到 stages 之间的差异
